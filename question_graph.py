...
class Evaluate(BaseNode[QuestionState]):
    ...
    async def run(
        ...
        print(ctx.state.evaluate_agent_messages)
...
class Congratulate(BaseNode[QuestionState, None, None]):
    ...
    async def run(self, ctx: GraphRunContext[QuestionState]) -> Annotated[End, Edge(label="success")]:
...
question_graph = Graph(nodes=(Ask, Answer, Evaluate, Congratulate, Reprimand), state_type=QuestionState)
...
async def run_as_cli(answer: str | None):
    ...
    history = question_graph.load_history(history_file.read_bytes()) if history_file.exists() else []
